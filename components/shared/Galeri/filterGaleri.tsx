...
import { galleryFilters, getFilteredItems, getCategoryCount, filterDescriptions } from "@/constants/Galeri/filterGallery";
import { GalleryFilter, FilteredGalleryItem } from "@/lib/type/Galeri/Galeri";

export default function FilterGaleri() {
    const [activeFilter, setActiveFilter] = useState<GalleryFilter>("All");
    const [filteredItems, setFilteredItems] = useState<FilteredGalleryItem[]>(getFilteredItems("All"));
    const [selectedItem, setSelectedItem] = useState<FilteredGalleryItem | null>(null);
        ...
        <main className="bg-[#4B061A] py-12 md:py-20">
            <div className="max-w-7xl mx-auto px-4 md:px-6">
                ...
                <div className="flex flex-row items-center gap-2 md:gap-3 mb-3 md:mb-4">
                    <Image src={FilterIcon} alt="Filter Icon" width={20} height={20} className="md:w-7 md:h-7" />
                    <div className="flex flex-row items-center gap-2 md:gap-3">
                        <p className="font-bold text-xl md:text-2xl tracking-widest text-white">Filter</p>                   
                    </div>
                </div>
                ...
                <div className="mb-6 md:mb-8">
                    <p className="text-white/70 text-xs md:text-sm">
                        <span className="text-[#A14CF3] font-semibold">PK</span> = Program Kerja
                    </p>
                </div>
                ...
                <div className="flex flex-wrap gap-2 md:gap-3 mb-6 md:mb-8">
                    {galleryFilters.map((filter) => (
                        ...
                        >
                            {filterDescriptions[filter].displayName} ({getCategoryCount(filter)})
                            
                            {/* Tooltip for all buttons - Hidden on mobile */}
                                ...
                                <div className="absolute bottom-2 left-2 right-2 md:bottom-4 md:left-4 md:right-4">
                                    ...
                                    <div className="space-y-0.5 md:space-y-1 text-white text-xs md:text-sm text-shadow-lg">
                                        <p className="flex items-center gap-1 md:gap-2">
                                            <span className="text-[#FFE8DB] font-bold text-xs flex-shrink-0">📅</span>
                                            <span className="font-medium truncate text-xs md:text-sm">{item.tanggal}</span>
                                        </p>
                                        <p className="flex items-center gap-1 md:gap-2">
                                            <span className="text-[#FFE8DB] font-bold text-xs flex-shrink-0">📍</span>
                                            <span className="font-medium truncate text-xs md:text-sm">{item.lokasi}</span>
                                        </p>
                                    </div>
                                </div>

                                {/* Fallback Detail Button - Always visible on mobile */}
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-white/70 text-sm">
                                            {filterDescriptions[selectedItem.category as GalleryFilter].description}
                                        </p>
                                    </div>
                                    ...
                                    <div className="space-y-4">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-white font-medium">{selectedItem.tanggal}</p>
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-white font-medium">{selectedItem.lokasi}</p>
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex items-start gap-3">
                                            ...
                                            <div className="flex-1">
                                                ...
                                                <p className="text-white font-medium leading-relaxed">{selectedItem.alamatLengkap}</p>
                                            </div>
                                        </div>
